...
def containerize(workflow, dag):
    ...
    def relfile(env):
        if isinstance(env.file, LocalSourceFile):
            return os.path.relpath(env.file.get_path_or_uri(secret_free=False), os.getcwd())
        else:
    ...
    for env in envs:
        ...
        if isinstance(env.file, LocalSourceFile):
            ...
            get_env_cmds.append(f"ADD {env.file.get_path_or_uri(secret_free=True)} {env_target_path}")
