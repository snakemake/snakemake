...
Importantly, the wildcard names in input and output must be named identically. It is of course also possible to have wildcards only in the output but not the input section or the other way around.
...
Starting from version 3.7, threads can also be a callable that returns an ``int`` value. It is also possible to refer to a predefined variable (e.g, ``threads: threadsMax``) so that the number of cores for a set of rules can be changed with one change only by altering the value of the variable ``threadsMax``.
...
.. code-block:: python
    ...
    rule:
        ...
        params: 
            ...
            RGID = lambda wildcards: RGFields[wildcards.sample]["ID"], # RGFields is a predefined array that assign a read group ID for each sample
        shell:  "somecommand -o {params.prefix} \
