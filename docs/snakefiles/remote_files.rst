...
It is possible to use S3-compatible storage by specifying a different endpoint address as the `host` kwarg in the provider, as the kwargs used in instantiating the provider are passed in to [`boto`](https://boto.readthedocs.org/en/latest/ref/s3.html#boto.s3.connection.S3Connection):
...
The remote provider also supports a new ``glob_wildcards()`` (see [this FAQ entry](https://bitbucket.org/snakemake/snakemake/wiki/FAQ#markdown-header-how-do-i-run-my-rule-on-all-files-of-a-certain-directory)) which acts the same as the local version of `glob_wildcards()`, but for remote files:
...
For different types of authentication, you can pass in a Python ```requests.auth`` object <http://docs.python-requests.org/en/latest/api/#authentication>`_ the `auth` ``kwarg``.
