...
.. sidebar:: Note
   ...
   The reason is that the ``run`` directive has access to the rest of the Snakefile (e.g. globally defined variables) and therefore must be executed in the same process as Snakemake itself. If used with ``notebook`` directive, the associated conda environment should have package ``nbconvert`` installed (this package is used to execute the notebook).
