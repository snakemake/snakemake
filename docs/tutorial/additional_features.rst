...
Here, we will talk about some additional features that may be useful for your workflow.
For details and even more features, see :ref:`snakefiles-rules`, :ref:`project_info-faq` and the command line help (``snakemake --help``).
...
In this tutorial, you have already seen how Conda_ can be used to specify an isolated software environment for a whole workflow.
...
It is best practice to specify at least the `major and minor version <https://semver.org/>`_ of any packages in the environment definition.
...
Second, a workflow can be re-executed (without admin rights) on a vanilla system, without installing any prerequisites apart from Snakemake and Miniconda_.


Tool wrappers
:::::::::::::

In order to simplify the utilization of popular tools, Snakemake provides a repository of so-called *wrappers* (the `Snakemake wrapper repository <https://snakemake-wrappers.readthedocs.io>`_).
