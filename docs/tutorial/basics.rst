...
Nevertheless, we provide some background in the following section.
...
Certain genomic differences can cause syndromes or predisposition for certain diseases, or cause cancerous growth in the case of tumour cells that have accumulated changes with respect to healthy cells.
...
For technical reasons, DNA sequencing cuts the DNA of a sample into millions of smaller pieces, measured as so-called *reads*.
...
They are responsible for harmless individual properties (like eye color), but can also cause diseases like cancer.
...
:oldanchor:`step-1-mapping-reads`
...
:oldanchor:`step-2-generalizing-the-read-mapping-rule`
...
.. note::
  You can have multiple wildcards in your output and input file paths.
...
:oldheading:`Step 4: Indexing read alignments and visualizing the DAG of jobs`
...
Next, we first need to use samtools_ again to index the sorted read alignments so that we can quickly access reads by the genomic location they were mapped to.
...
:oldheading:`Step 5: Calling genomic variants`
