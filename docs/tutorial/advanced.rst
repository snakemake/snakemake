...
when multiple jobs are run in parallel, this would result in chaotic output.
...
We modify the rule ``samtools_sort`` to mark its output, the final BAM file, as ``protected``:
