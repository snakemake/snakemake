...
def job_to_cwl(job, dag, outputs, inputs):
    ...
    out = [get_output_id(job, i)
           for i, _ in enumerate(job.output)]
    ...
    cwl = {
        ...
        "in": {
            ...
            "target_files": {
                "default": job.output.plainstrings()
            },
            "rules": {
    ...
    input_files.extend(
        "#main/job-{}/output_files".format(dep.jobid)
...
def dag_to_cwl(dag):
    ...
    snakemake_cwl = {
        ...
        "arguments": [
            ...
            "--keep-remote",
            "--force-use-threads",
            "--wrapper-prefix", dag.workflow.wrapper_prefix,
            "--notemp",
            "--quiet",
            "--use-conda",
            "--no-hooks",
            "--nolock",
            "--mode", str(Mode.subprocess)
        ],
        "inputs": {
            "snakefile": {
                "type": "File",
    ...
    dag_cwl = [job_to_cwl(job, dag, outputs, inputs)
               for job in groups]
