...
class ProfileConfigFileParser(YAMLConfigFileParser):
    def parse(self, stream):
        ...
        def format_two_level_dict(d):
            return [f"{key}:{key2}={val2}" for key, val in d.items() for key2, val2 in val.items()]
        ...
        for key, value in parsed_obj.items():
            if isinstance(value, list):
                ...
                if isinstance(value, (dict, OrderedDict)):
                    if key in ("set-threads", "resources", "set-scatter", "batch", "set-resource-scopes", "default-resources", "config", "wms-monitor-arg", ):
