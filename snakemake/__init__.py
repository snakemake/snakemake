...
def snakemake(
    ...
    Args:
        ...
        list_params_changes (bool): list output files (default False)
        ...
        aws_batch (bool):           Execute workflow tasks on aws batch.
        aws_batch_fsap_id (str):       The fsap id of the EFS instance you want to use for Batch that is shared with your local environment
        aws_batch_efs_project_path (str):       The path on the EFS instance to the project directory.
        aws_batch_task_queue (str):       The AWS Batch task queue name that is used for running the individual tasks
        aws_batch_workflow_role (str):    The AWS IAM role that is used for running the AWS Batch tasks expressed as an ARN
        aws_batch_task_timeout (int):    The timeout to use when executing the tasks via AWS Batch
        aws_batch_data_already_uploaded (bool):    The project already exists in the S3 location provided via the default_remote_prefix prefix
    ...
    # AWS Batch uses EFS for shared storage for workflows
    if aws_batch:
        assume_shared_fs = True
...
def get_argument_parser(profiles=None):
    ...
    group_utils.add_argument(
        ...
        "--lo",
        action="store_true",
        help="List all output files in the working directory that are used in the  "
        "workflow.",
    )
    group_utils.add_argument(
        "--delete-all-output",
    ...
    group_aws_batch = parser.add_argument_group("AWS_BATCH")
    ...
    group_aws_batch.add_argument(
        "--aws-batch",
        action="store_true",
        help="Execute workflow on AWS using AWS Batch. This requires default "
             " AWS CLI credentials to be defined in your environment along with the "
             " region being exported as AWS_REGION in your environment. "
    ...
    group_aws_batch.add_argument(
        "--aws-batch-fsap-id",
        help="The EFS instance's FSAP ID that will be used for storing data when running the workflow",
    )

    group_aws_batch.add_argument(
        "--aws-batch-efs-project-path",
        help="The EFS path that contains the project that Snakemake is running",
    )

    group_aws_batch.add_argument(
        "--aws-batch-task-queue",
        help="The AWS Batch task queue ARN that is used for running the individual tasks"
             " inside Snakemake.",
    )

    group_aws_batch.add_argument(
        "--aws-batch-workflow-role",
    ...
    group_aws_batch.add_argument(
        ...
        nargs="*",
        metavar="KEY=VALUE",
        help="The tags that should be applied to all of the batch tasks that are executed",
    )

    group_aws_batch.add_argument(
        "--aws-batch-task-timeout",
        type=int,
        help="Supplying a value will force AWS Batch to terminate "
             "a Batch task if it fails to finish within the"
             "determined time. It is calculated in seconds and the minimum value is 60.",
    )

    group_aws_batch.add_argument(
        "--aws-batch-data-already-uploaded",
        action="store_true",
        help="A flag that determines whether Snakemake should upload "
             "the data to S3 before running the workflow",
    )
...
def main(argv=None):
    ...
    aws_batch_tags = dict()
    if args.aws_batch:
        if not args.default_remote_prefix and not args.aws_batch_fsap_id:
            print(
                "Error: --aws-batch must be combined with --default-remote-prefix "
                "to provide bucket name and subdirectory (prefix) "
                "(e.g. 'bucketname/projectname'",
                file=sys.stderr,
            )
        ...
        if args.aws_batch_tags:
            errmsg = "Invalid --aws-batch-tags supplied: entries have to be defined as KEY=VALUE pairs"
            for tag in args.aws_batch_tags:
                key, value = parse_key_value_arg(tag, errmsg)
                aws_batch_tags[key] = value

        if args.aws_batch_fsap_id:
            if not args.aws_batch_fsap_id.startswith("fsap-"):
                print(
                    "Error: --aws-batch-fsap-id must begin with fsap-",
                    file=sys.stderr,
                )
                sys.exit(1)
            if not args.aws_batch_efs_project_path:
                print(
                    "Error: --aws-batch-efs-project-path must be present with fsap-id",
                    file=sys.stderr,
                )
                sys.exit(1)

        if not args.aws_batch_task_queue:
            print(
                "Error: --aws-batch-task-queue must be supplied",
                file=sys.stderr,
            )
            sys.exit(1)

        if not args.aws_batch_workflow_role:
            print(
                "Error: --aws-batch-workflow-role must be supplied",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if args.gui is not None:
        ...
        success = snakemake(
            ...
            tes=args.tes,
            aws_batch=args.aws_batch,
            aws_batch_fsap_id=args.aws_batch_fsap_id,
            aws_batch_efs_project_path=args.aws_batch_efs_project_path,
            aws_batch_task_queue=args.aws_batch_task_queue,
            aws_batch_task_timeout=args.aws_batch_task_timeout,
            aws_batch_data_already_uploaded=args.aws_batch_data_already_uploaded,
            aws_batch_tags=aws_batch_tags,
            aws_batch_workflow_role=args.aws_batch_workflow_role,
