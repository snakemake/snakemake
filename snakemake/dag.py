...
class DAG(DAGExecutorInterface):
    ...
    async def retrieve_storage_inputs(self):
        ...
        if (self.workflow.is_main_process and shared_local_copies) or (
            ...
            to_retrieve = {
                ...
                for job in self.needrun_jobs()
                for f in job.input
                if f.is_storage and self.is_external_input(f, job, not_needrun_is_external=True)
            }
    ...
    def is_external_input(self, file, job, not_needrun_is_external=False):
        ...
        return not any(file in files for dep, files in self.dependencies[job].items() if consider(dep))
