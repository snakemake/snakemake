...
class SlurmJobstepExecutor(ClusterExecutor):
    ...
    def run(self, job, callback=None, submit_callback=None,
            error_callback=None):
        super()._run(job)
        ...
        if job.is_group():
            def get_call(level_job, level_id, aux=''): 
                ...
                return 'srun -J {level_id} --jobid {jobid} --mem-per-cpu {mem_per_cpu} -c {level_job.threads} --exclusive -n 1 {aux} {exec_job}'.format(level_id = level_id, level_job = level_job, aux = aux, jobid = self.jobid, exec_job = self.format_job(level_job))
            for level in job.toposorted:
                for level_id, level_job in enumerate(level[:-1]):
                    #TODO: spice with srun template
                    jobsteps[level_job] = subprocess.Popen(get_call(level_job,level_id), shell=True)
                # this way, we ensure that level jobs depending on the current level get started
                jobsteps[level_job] = subprocess.Popen(get_call(level_job,level_id+1,aux='--singleton'), shell=True))
