...
def lint_rules(rules):
    for rule in rules:
        ...
        if rule_lints:
            logger.info("Lints for rule {} (line {}, {}):\n{}\n".format(rule.name, rule.lineno, rule.snakefile, "\n".join(map("    * {}".format, rule_lints))))
    ...
    for param, value in rule.params.items():
        if isinstance(value, str):
            if any(f.startswith(value) for f in chain(rule.input, rule.output)):
                yield Lint(
                    title="Param {} is a prefix of input or output file but hardcoded".format(param), 
                    ...
                    links=["https://snakemake.readthedocs.io/en/stable/snakefiles/rules.html#non-file-parameters-for-rules", "https://snakemake.readthedocs.io/en/stable/tutorial/advanced.html#tutorial-input-functions"]
                )
...
def lint_log_directive(rule):
    if not rule.log and not rule.norun:
        yield Lint(
            ...
            links=["https://snakemake.readthedocs.io/en/stable/snakefiles/rules.html#log-files"]
        )
        ...
        for name in get_wildcard_names(rule.shellcmd):
            if name not in valid_names:
                yield Lint(
                    ...
                    links=["https://snakemake.readthedocs.io/en/stable/snakefiles/rules.html#non-file-parameters-for-rules"]
                )
...
class Lint:
    ...
    def __str__(self):
        ...
        return "{}:\n{}\n      Also see:\n{}".format(self.title, "\n".join(map("      {}".format, textwrap.wrap(self.body, max(width - 6, 20)))), "\n".join(map("      {}".format, self.links)))

lints = [func for name, func in globals().items() if name.startswith("lint_") and not name == "lint_rules"]
