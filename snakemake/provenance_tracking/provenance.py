...
class PROVMgr(object):
    ...
    def __init__(self):
        ...
        self.document.add_namespace('crypto', 'http://id.loc.gov/vocabulary/preservation/cryptographicHashFunctions#')
    ...
    def add_activity(self, tool_name, job_uri, input_id_list, cmd):
        activity = self.bundle.activity(identifier=str(job_uri), startTime=self.__get_iso_now)
        ...
        for in_id in input_id_list:
            ...
            try:
                if os.path.isfile(os.path.abspath(in_id)):
                    digest = hashlib.sha512(open(os.path.abspath(in_id), 'rb').read()).hexdigest()
                    input.add_attributes({"crypto:sha512": digest})
            except Exception as e:
                logger.error("Error while computing the SHA512 fingerprint for " + str(in_id))
    ...
    def add_output(self, output_id, input_id_list, tool_name, job_uri):
        ...
        try :
            ...
            logger.error("Error while computing the SHA512 fingerprint for "+str(output_id))
